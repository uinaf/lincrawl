...
- Tenant-specific operating state belongs in the put.io-owned crawl store
  (`putdotio/putio-lincrawl-store`), not here.

## Local secrets

Locally, prefer the user's 1Password CLI integration. Verify with
`op whoami --account=putdotio.1password.com`. This repo's verify works without
